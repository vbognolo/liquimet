...
<body>
    <header id="header" class="header d-flex align-items-center justify-content-center">
        ...
        <?php foreach ($navigation as $navbar){ ?>
            <li class="vr"></li>

            <li class="nav-item nav-justified nav-fill mx-auto my-auto">       
                <a class="nav-link w-100" id="platform-tab" aria-controls="nav-platform"
                    href="category.php?id_group=<?= $navbar['id_group'] ?>"
                        <?= ($group == $navbar['id_group']) ? 'class="active" aria-current="page"' : ' ' ?>>
                        <?= html_escape($navbar['nav']) ?>
                </a>
            </li>
